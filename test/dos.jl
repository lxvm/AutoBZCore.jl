...
for (model, solution, bandwidth, bzkind) in (
    ...
    for alg in (GGR(; npt = 200), ImplicitIntegrationJL(), BCD(; npt = 50), LT(; npt=200),)
        alg isa LT && !(bzkind isa FBZ) && continue
        alg isa ImplicitIntegrationJL && !(bzkind isa FBZ) && continue
        cache = AutoBZCore.init(prob, alg; abstol = 1e-2)
        @testset "model=$(nameof(solution)) alg=$alg E=$e" for e in E
            cache.domain = e
